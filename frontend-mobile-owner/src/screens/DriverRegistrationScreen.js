...
export default function DriverRegistrationScreen({ navigation }) {
  ...
  async function submit() {
    ...
    try {
      setLoading(true);
      // create user first
      const userResp = await createUser({ tenantId: null, email, phone, password, role: 'Driver', isActive: true });
      const userId = userResp?.id || userResp?.userId || userResp?.data?.id;
      await createDriverProfile({ userId: userId || null, name, idNumber, phone, email, photoUrl: photoBase64, licenseCopy: photoBase64 });
