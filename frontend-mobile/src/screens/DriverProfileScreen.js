...
export default function DriverProfileScreen({ navigation, route }) {
  ...
  const loadActiveTrips = async () => {
    try {
      ...
      const res = await client.get(`/TaxiRankTrips/driver/${driverId}/active`);
      setActiveTrips(Array.isArray(res.data) ? res.data : []);
    } catch (e) {
      console.error('Load active trips error:', e);
      setActiveTrips([]);
    } finally {
      ...
      >
        ...
        <View style={[styles.card, { backgroundColor: c.surface, borderColor: c.border }]}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.cardTitle, { color: c.text }]}>Active Trips</Text>
            <View style={[styles.tripCountBadge, { backgroundColor: '#3b82f6' }]}>
              <Text style={styles.tripCountText}>{activeTrips.length}</Text>
            </View>
          </View>
          ...
          {loadingTrips ? (
            ...
            <View style={styles.centerContent}>
              <Ionicons name="car-outline" size={48} color={c.textMuted} style={{ opacity: 0.5 }} />
              <Text style={[styles.emptyText, { color: c.textMuted }]}>No active trips</Text>
            </View>
          ) : (
            activeTrips.map((trip) => (
              <View key={trip.id} style={[styles.tripItem, { backgroundColor: c.background }]}>
                <View style={styles.tripHeader}>
                  <View style={styles.tripStatusContainer}>
                    <View style={[styles.tripStatusDot, { backgroundColor: getTripStatusColor(trip.status) }]} />
                    <Text style={[styles.tripStatus, { color: getTripStatusColor(trip.status) }]}>
                      {(trip.status === 'Departed' || trip.status === 'Dispatched') ? 'EN ROUTE' : (trip.status || 'In Progress')}
                    </Text>
                  </View>
                  {trip.departureTime && (
                    <Text style={[styles.tripTime, { color: c.textMuted }]}>
                ...
                <View style={styles.tripRoute}>
                  <View style={styles.routePoint}>
                    ...
                    <Text style={[styles.routeText, { color: c.text }]} numberOfLines={1}>
                      {trip.departureStation || trip.origin || 'Departure'}
                    </Text>
                  </View>
                  ...
                  <View style={styles.routePoint}>
                    ...
                    <Text style={[styles.routeText, { color: c.text }]} numberOfLines={1}>
                      {trip.destinationStation || trip.destination || 'Destination'}
                    </Text>
                  </View>
                </View>
                ...
                <View style={styles.tripMeta}>
                  {trip.vehicle?.registration && (
                    <View style={styles.metaItem}>
                      <Ionicons name="car" size={14} color={c.textMuted} />
...
const styles = StyleSheet.create({
  ...
  tripCountText: { fontSize: 12, fontWeight: '700', color: '#fff' },
  ...
  tripTime: { fontSize: 11, fontWeight: '500' },
  ...
  routeText: { fontSize: 13, marginLeft: 6, flex: 1 },
