...
export default function TripManagementScreen({ navigation, route }) {
                    ...
                    onPress: async () => {
                      try {
                        ...
                        setQueueData(prevQueue =>
                          prevQueue.map(q =>
                            q.id === entry.id
                              ? { ...q, status: 'Dispatched', departedAt: new Date().toISOString() }
                              : q
                          )
                        );
  ...
  const handleRemoveVehicle = async (queueEntry) => {
    const queueEntryId = queueEntry?.id || queueEntry?.Id;
    const vehicleReg = queueEntry?.vehicleRegistration || queueEntry?.vehicle?.registration || 'vehicle';

    if (!queueEntryId) {
      Alert.alert('Error', 'Invalid queue entry ID');
      return;
    }
    ...
    const doRemove = async () => {
      ...
      try {
        ...
        Alert.alert('Error', error?.response?.data?.message || error?.message || 'Failed to remove vehicle');
      } finally {
    ...
    openConfirmDialog({
      ...
      message: `Remove ${vehicleReg} from the queue?`,
      onConfirm: doRemove,
    });
  };
  ...
  const handleCompleteTrip = async (trip) => {
    ...
    const vehicleReg = trip?.vehicleRegistration || trip?.vehicle?.registration || 'vehicle';
  ...
  return (
    <View style={[s.root, { paddingTop: insets.top }]}>  
      ...
      <Modal visible={confirmVisible} transparent animationType="fade" onRequestClose={() => !confirmBusy && setConfirmVisible(false)}>
        <View style={s.modalBgCenter}>
          <View style={s.confirmCard}>
            ...
            <View style={s.confirmActions}>
              <TouchableOpacity
                style={[s.addBtn, s.confirmCancelBtn]}
                onPress={() => setConfirmVisible(false)}
                disabled={confirmBusy}
              >
                <Text style={[s.addBtnTxt, s.confirmCancelTxt]}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[s.addBtn, s.confirmDangerBtn]}
