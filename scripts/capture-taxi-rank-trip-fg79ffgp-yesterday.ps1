# Script to capture a taxi rank trip for vehicle FG79FFGP with 15 passengers for yesterday's date
# This script creates a Trip record linked to the Dundonald Taxi Rank through vehicle association

Write-Host "=== Mzansi Fleet Taxi Rank Trip Capture Script (Yesterday) ===" -ForegroundColor Cyan
Write-Host "Vehicle: FG79FFGP | Passengers: 15 | Taxi Rank: Dundonald | Date: Yesterday" -ForegroundColor Yellow
Write-Host ""
...
# Step 4: Create trip data for TripDetails endpoint (yesterday's date)
Write-Host "Step 4: Creating trip data for yesterday..." -ForegroundColor Green

$yesterday = (Get-Date).AddDays(-1)
$tripData = @{
    vehicleId = $vehicleId
    tripDate = $yesterday.ToString("yyyy-MM-ddTHH:mm:ssZ")
    departureTime = "14:30"  # Different departure time for yesterday's trip
    passengerCount = $passengers.Count
    totalFare = $totalFare
    status = "Completed"
    notes = "Taxi rank trip captured via automated script - Vehicle $vehicleRegistration with $($passengers.Count) passengers linked to Dundonald Taxi Rank (Yesterday: $($yesterday.ToString('yyyy-MM-dd')))"
...
Write-Host "Trip for yesterday ($($yesterday.ToString('yyyy-MM-dd'))) is now linked to the Dundonald Taxi Rank through vehicle association and ready for management." -ForegroundColor Green